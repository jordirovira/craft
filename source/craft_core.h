...
{
public:
    Task( const std::string& type, const NodeList& outputs, std::function<void(Context*)> run )
        : m_type(type)
        , m_outputs(outputs)
        , m_runMethod(run)
    {
    ...
    Task( const std::string& type, std::shared_ptr<Node>& output, std::function<void(Context*)> run )
        : m_type(type)
        , m_runMethod(run)
    {
    ...
    std::string m_type;
    NodeList m_outputs;
    std::function<void(Context*)> m_runMethod;
